=== Popup with exit intent, scroll triggered and anchor click for opt-ins, lead gen & more ===
Contributors:      tropicalista
Tags:              popup, block, popup exit intent, exit popup, popup builder
Requires at least: 6.6
Tested up to:      6.6
Stable tag:        0.7.9
Requires PHP:      5.6.0
License:           GPL-2.0-or-later
License URI:       https://www.gnu.org/licenses/gpl-2.0.html

A popup builder to increase Sales, Lead Generation, Conversion rates and receive good Call to Action with exit intent.

== Description ==

Popper helps you create a variety of **popups** for any circumstances. You can display a **popup** everywhere, choose conditions where to display, add multiple popup. 
With Popper **popup builder** you can create high converting, promotional and informative popups, increase conversion rates and boost sales while reaching your marketing goals.
You can use all available Gutenberg blocks to build your own popup content. If you want a form to collect leads, you can use [Formello](https://formello.net).

Popper has a nice set of animation ready to use and you can also choose different position on the screen. For some example please visit the [Popper page](https://formello.net/addons/popper).

== FEATURES ==

* Open popup on exit intent ( Experimental on mobile )
* Open popup on anchor click
* Open popup on adblock detect
* Open popup on scroll percent
* Open popup after time amount
* Open popup on target visibility
* Open popup after number of pages visits
* Customizable popup border radius and box shadow with presets
* Display rules based on page/post/taxonomy or user roles

**DOCUMENTATION**

Here you can find [documentation](https://docs.formello.net/addons/popper).

## Privacy Policy 
Popup with exit intent, scroll triggered and anchor click for opt-ins, lead gen &amp; more uses [Appsero](https://appsero.com) SDK to collect some telemetry data upon user's confirmation. This helps us to troubleshoot problems faster & make product improvements.

Appsero SDK **does not gather any data by default.** The SDK only starts gathering basic telemetry data **when a user allows it via the admin notice**. We collect the data to ensure a great user experience for all our users. 

Integrating Appsero SDK **DOES NOT IMMEDIATELY** start gathering data, **without confirmation from users in any case.**

Learn more about how [Appsero collects and uses this data](https://appsero.com/privacy-policy/).

== WHAT’S NEXT ==

If you like this plugin, then consider checking out our other projects:

* [Formello](https://wordpress.org/plugins/formello): a form builder to collect leads, newsletter signup, contact form and more.
* [Search Console](https://wordpress.org/plugins/search-console): view all your search console data inside WordPress admin.
* [Mortgage Calculator](https://wordpress.org/plugins/mortgage): a mortgage calculator block for Gutenberg.
* [Pdf Embed](https://wordpress.org/plugins/pdf-embed): a simple block for Gutenberg to embed a PDF using official Adobe Embed API.


**CONTRIBUTE**

Popper is open source and you can [contribute here](https://github.com/formello-forms/popper).

== Installation ==

1. Upload the plugin files to the `/wp-content/plugins/popper` directory, or install the plugin through the WordPress plugins screen directly.
2. Activate the plugin through the 'Plugins' screen in WordPress


== Frequently Asked Questions ==

= Will this slow down my site? =

No. Popper has no external dependencies and does not require jQuery. Only 2kb(gzipped) of js is loaded.

= Is exit intent available? =

Yes. We have integrated exit-intent for desktop at the moment. We're planning to add also exit-intent for mobile.

= Is required jQuery? =

No. No external library is required. We mantain a clean fast code without dependency.

== Screenshots ==

1. Popup creation.
2. Popper settings
3. Popper block selection
4. Popper presets
5. Popper block settings
6. Popper display settings

== Changelog ==

= 0.7.9 =
* Fix textarea
* Small css fixes
* WP 6.6 compatibility

= 0.7.8 =
* Fix dataviews import
* Prepare for new WP 6.6 release
* Fix button handling
* Minor CSS issues

= 0.7.7 =
* Added warning on modal
* Added button to start/stop

= 0.7.6 =
* Added notice on missing locations and device
* Fix modal
* Fix row selection

= 0.7.5 =
* Fix date selection

= 0.7.4 =
* Small fixes

= 0.7.3 =
* Fix date conditions

= 0.7.2 =
* Remove shadow on closed modal
* Fix datatable bulk Action
* Fix layout css
* Fix wrong namespace
* Added running status in dataviews

= 0.7.1 =
* Fix full height on mobile browser

= 0.7.0 =
* Added popup body block
* Better handling of style variations
* Added field constraints
* Fix redirect form submissions
* Switched to style variations
* Updated patterns

= 0.6.3 =
* Fix scroll behaviour
* Fix close on form for fluent form

= 0.6.2 =
* Fixed devices settings
* Fixed editor css

= 0.6.1 =
* Fixed devices settings
* Fixed editor css

= 0.6.0 =
* Add support for datatables
* Added modal to edit conditions from admin
* Added patterns
* Added form
* Added integrations
* Added export patterns

= 0.5.4 =
* Fix display button for Formello form

= 0.5.3 =
* Fix custom time

= 0.5.2 =
* Added custom time
* Added custom day of week

= 0.5.1 =
* Fix category condition

= 0.5.0 =
* Added Appsero

= 0.4.9 =
* Fix error on iframe removing
* Added full width option
* Removed content filter

= 0.4.8 =
* Better handling of selection
* Fix editor css
* Added adblock trigger
* Removed lock to fix block recovery. Bug added to Gutenberg https://github.com/WordPress/gutenberg/issues/49005

= 0.4.7 =
* Fixes modal
* better handling of column on popup

= 0.4.5 =
* Fixes dates

= 0.4.4 =
* Fixes pattern transient expiration

= 0.4.3 =
* Fixes devices attributes
* Hide popup block from inserter

= 0.4.2 =
* Better handling of remote template
* Added scroll hide
* Added close button block
* Code clean up
* Better handling of templates using pattern modal

= 0.4.1 =
* Fix missing date meta
* Added scroll hide

= 0.4.0 =
* Fix version number

= 0.3.9 =
* Better handling of borders using Gutenberg default controls
* Better css for long scrollable popup
* Added date support
* Added button position
* Switch to svg icon for closing button
* Added background and color options for buttons

= 0.3.8 =
* Fix device error

= 0.3.7 =
* Fix wrong columns names
* Added device targeting
* Added auto include popup on page on anchor and scroll on element target ID

= 0.3.6 =
* Fix wp tested version

= 0.3.5 =
* Fix wp version

= 0.3.4 =
* Added trigger on table

= 0.3.3 =
* Better animation handling

= 0.3.1 =
* Fix overlay error
* Better animation handling

= 0.3.1 =
* Fix html output

= 0.3.0 =
* Fix css

= 0.2.9 =
* Fix template size
* Better fullscreen support

= 0.2.8 =
* Bump

= 0.2.7 =
* Added default color
* Better style management

= 0.2.6 =
* Better handling css
* Added column to show display rules

= 0.2.5 =
* Prepare for WP 5.9

= 0.2.4 =
* Fix content not selectable
* Add templates

= 0.2.3 =
* Prevent opening multiple popups

= 0.2.2 =
* Fix missing id on modal

= 0.2.1 =
* Fix asset not loading on frontend

= 0.2.0 =
* Code improvements
* Code reduction
* Better controls
* New animations
* Added new positions

= 0.1.9 =
* Added animations
* Added bottom slide
* Added button positions
* Preparing for next Wp release
* Fixed users rules

= 0.1.7 =
* Fixed exit intent
* Fixed content display
* Added some presets
* Better sql handling

= 0.1.6 =
* Fixed location rule

= 0.1.5 =
* Added experimental support for exit intent on mobile

= 0.1.4 =
* Added better assets

= 0.1.3 =
* Fixed rules
* Added modal control on block

= 0.1.2 =
* Added translations

= 0.1.1 =
* Added correct links

= 0.1.0 =
* Release
